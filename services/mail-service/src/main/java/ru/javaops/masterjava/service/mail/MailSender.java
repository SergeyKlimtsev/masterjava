...
public class MailSender {
    ...
    static MailResult sendTo(Addressee to, String subject, String body, List<Attachment> attachment) throws WebStateException {
        val state = sendToGroup(ImmutableSet.of(to), ImmutableSet.of(), subject, body, attachment);
        return new MailResult(to.getEmail(), state);
    }

    static String sendToGroup(Set<Addressee> to, Set<Addressee> cc, String subject, String body, List<Attachment> attachment) throws WebStateException {
        ...
        try {
            ...
            if (attachment != null) {
                for (Attachment attach : attachment) {
                    DataHandler data = attach.getData();
                    email.attach(data.getDataSource(), attach.getName(), data.getContentType());
                }
            }
